...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    console.log('Checking token in Dashboard:', token ? 'Found' : 'Not Found');
    ...
    console.log('Allowed Guilds:', guilds);
    
    if (guilds.length > 0 && typeof guilds[0] === 'string') {
      console.log('Old security schema detected, clearing token');
      localStorage.removeItem('zenith_token');
      alert('Security schema updated. Please log in again.');
      return navigate('/login');
    }

    if (guilds.length === 0 && !payload.isOwner) {
      console.log('No guilds and not owner, access denied');
